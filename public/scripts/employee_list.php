...
$joinQuery = "FROM `employees` AS `u`
			...
			LEFT JOIN `tbl_company_branch` AS `br` ON `br`.`idtbl_company_branch`=`u`.`emp_branch`
			LEFT JOIN `employment_statuses` AS `st` ON `st`.`id`=`u`.`emp_status`
			LEFT JOIN `users` AS `us` ON `us`.`emp_id`=`u`.`id`";
